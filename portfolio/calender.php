...
try{
  $user = 'root';
  $pass = 'root';
	$pdo = new PDO('mysql:dbname=choenji; host=localhost; charset=utf8',$user,$pass,
		array(
...
function display_to_Schedule($date,$schedule_array) {
	//予定データ配列の中に、引数の日付がキーのデータが入ってるか調べます
	if(array_key_exists($date,$schedule_array)){
		//引数の日付がキーのデータが入っていたら
		$schedule = "<br/>".$schedule_array[$date];//一行改行、予定名を代入
		return $schedule; 
	}
}
...
function getHolidays($year) {
	...
	$api_key = 'AIzaSyA1fRRP2vBRGv6NC4X3dzzItTVdZ9rf7r0';
	$holidays = array();
	$holidays_id = 'australian__en@holiday.calendar.google.com'; // Google 公式版日本語
...
try {
    // SQL文作成
    $stmt = $pdo->prepare('SELECT * FROM schedule  WHERE schedule_date BETWEEN :month_fast_day AND :next_month_fast_day
			ORDER BY schedule_date');
    
	$stmt -> bindParam(':month_fast_day', $display_date, PDO::PARAM_STR);
	$stmt -> bindParam(':next_month_fast_day', date("Y-m-d",strtotime('+1 month',strtotime($display_date))), PDO::PARAM_STR);
	$stmt -> execute();//実行
	if ( $stmt1 == false ){
		//print 'SELECTしましたがユーザーデータが一件もありません';
	}
} catch (Exception $e) {
	exit('データベース検索に失敗しました。'.$e->getMessage());
}
$schedule_array = array();//当日の勤務者名を入れる配列初期化
while($row = $stmt -> fetch(PDO::FETCH_ASSOC)) { /*スタッフの人数の一ヶ月の勤務日分繰り返す */
	$schedule_array[$row['schedule_date']] .= "<br/>".$row['eventname'];//一行改行、勤務者名を代入
}
...
<body>
    ...
    <div>
        <!-- 何月を表示 -->
        <!-- 一ヶ月前のページへのリンク -->
        <a href="index.php?first_day_of_month=<?php echo date('Y-m-01',strtotime("-1 month",strtotime($display_date))); ?>#schedule">
            <<</a> <!-- /一ヶ月前のページへのリンク -->
                <?php echo date('Y-n',strtotime($display_date)); ?>
                <!-- 一ヶ月後のページへのリンク -->
                <a href="index.php?first_day_of_month=<?php echo date('Y-m-01',strtotime("+1 month",strtotime($display_date))); ?>#schedule">>></a>
                <!-- /一ヶ月後のページへのリンク -->
                <br />
...
?>
                        <td name=<?php echo $display_tb_name_array[$i]; //name属性付与 ?> id=<?php 
...
.display_to_Schedule(date("Y-m-d",strtotime($display_year."-".$display_month."-".$display_day_array[$ii])),$schedule_array);//当日、勤務予定なら名前を表示する関数 function.phpの関数
			$i++;
			$ii++;
